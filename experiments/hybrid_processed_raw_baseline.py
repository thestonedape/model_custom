"""
Hybrid processed+raw EEG word classification baseline.
"""
...
from data.hybrid_word_torch_dataset import HybridWordCacheDataset, collate_hybrid_word_batch
from models import HybridProcessedRawWordClassifier
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a hybrid processed+raw EEG baseline")
    parser.add_argument("--config", type=str, default="config/hybrid_processed_raw_smoke.yaml")
...
def create_loader(
    ...
    dataset = HybridWordCacheDataset(
    ...
    loader = DataLoader(
        ...
        collate_fn=collate_hybrid_word_batch,
    )
...
def evaluate_loader(model, loader, device, tracker, max_batches=None, split_name="Dev", shard_name=None):
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(loader):
            ...
            logits = model(
                processed_eeg=batch["processed_eeg"],
                eeg=batch["eeg"],
            ...
            tracker.update(
                logits,
                labels,
                {"L_total": float(loss.item()), "L_ce": float(loss.item()), "L_vq": 0.0, "L_cl": 0.0},
            )
...
def evaluate_split(
    ...
    for cache_path in cache_paths:
        ...
        while True:
            ...
            print(f"  {split_name} shard '{shard_name}': {len(dataset)} {granularity} samples (batch_size={current_batch_size})")
            if len(dataset) == 0:
                break
            try:
                evaluate_loader(
                ...
                print(f"  OOM during {split_name} on shard '{shard_name}'. Retrying with batch_size={next_batch_size}.")
...
def main():
    ...
    print("=" * 80)
    print("HYBRID PROCESSED+RAW BASELINE")
    print("=" * 80)
    ...
    if missing_paths:
        raise FileNotFoundError("Hybrid caches not found: " + ", ".join(missing_paths))
    ...
    model = HybridProcessedRawWordClassifier(
        processed_dim=config["model"].get("processed_dim", 840),
        eeg_dim=config["model"].get("eeg_dim", 105),
        et_dim=config["model"].get("et_dim", 4),
        metrics_dim=config["model"].get("metrics_dim", 5),
        model_dim=config["model"].get("model_dim", 192),
        processed_hidden_dim=config["model"].get("processed_hidden_dim", 256),
        ...
        ff_dim=config["model"].get("ff_dim", 384),
    ...
    if resume_path is not None:
        ...
        print(f"  Best dev Top-10 so far: {best_dev * 100:.2f}%")
        if start_epoch > total_epochs:
            print(f"Checkpoint already at epoch {start_epoch - 1}; nothing to run.")
            return
